...
import { ArrowLeft, ArrowRight, CircleNotch as Loader2, Eye, EyeSlash as EyeOff, UserPlus, SignIn as LogIn, ShieldCheck } from '@phosphor-icons/react'
...
export function AccesoPage() {
  ...
  const crearNegocio = async () => {
    ...
    if (!aceptaTerminos) { toast.error('Debes aceptar los Términos y la Política de Privacidad para continuar.'); return }
    setLoading(true)
    try {
      let resp
      if (googleCredential) {
        ...
        <div className="w-full max-w-md bg-white rounded-2xl shadow-2xl shadow-black/10 border border-gray-100 p-8 lg:max-w-sm lg:border-0 lg:shadow-none lg:rounded-none lg:p-0">
          ...
          {view === 'finalize' && (
            <motion.div key="finalize" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
              ...
              {/* Aceptación legal — casilla ÚNICA + declaración en letra pequeña */}
              ...
              <p className="mt-1.5 text-[11px] leading-relaxed text-gray-400">
                Al crear tu cuenta declaras que la información proporcionada es veraz, que cumplirás la legislación aplicable a tu actividad y que entiendes que Barber.pe es únicamente una plataforma tecnológica.{' '}
                <a href="/declaracion" target="_blank" rel="noopener noreferrer" className="font-medium text-blue-600 hover:underline">Ver declaración completa</a>.
              </p>

              <button onClick={crearNegocio} disabled={loading || !aceptaTerminos} className={btnPrimary + ' mt-3'}>
